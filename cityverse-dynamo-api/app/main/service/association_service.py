...
def edit_association(association_id, data, banner_image, profile_image):
    document = Document(__TABLE_NAME__='Association')
    ...
    if place:
        place.update({
            ...
            'banner_image': banner_image_url if banner_image_url is not None else place.get('banner_image'),
            'profile_image': profile_image_url if profile_image_url is not None else place.get('profile_image')
        })
