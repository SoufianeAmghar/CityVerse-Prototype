import React, {
  ...
  export default function ModalUpdateMission({ open, setOpen, idMission }) {
    ...
    const call_api_get_mission_by_id = (id) => {
      axios
        .get(process.env.REACT_APP_ADMINISTRATION_USERS_SERVER + "mission/" + idMission?.idMission)
        .then((value) => {

          setStartDate(value?.data?.start_date)
          setDesc(value?.data?.description)
          setNumVolunteers(value?.data?.number_of_participants)
          setMissionDuration(value?.data?.duration)
          setTypeMission(value?.data?.mission_type)
          setTypeVolunteers(value?.data?.volunteer_qualifications)         
        })
        .catch((err) => {      
        });
    };
    useEffect(() => {
      call_api_get_mission_by_id()
    }, [])
    const save = () => {
      const object = {
